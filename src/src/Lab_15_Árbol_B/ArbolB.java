...
class ArbolB{
    ...
    private int tamaño(NodoArbolB x){
        //Establece el int valor y aumenta según cuantos nodos valores insertados contiene.
        int valor = 0;
        for(int j=0; j<x.n; j++){
            valor++;
        }
        /*Comprueba si el nodo del árbol actual no es un nodo hoja, en caso de que no lo sea
        y analizará cada hijo del nodo y sumará los valores que poseen cada uno con recursividad
         */
        if(!x.leaf){
            for(int i=0; i<=x.n; i++){
                if(x.child[i]!=null){
        ...
        //retornara el resultado de la suma total
        return valor;
    }
    /*Método para calcular el porcentaje de utilización del árbol b, según cuantos espacios
    de cada nodo existente están siendo usados
     */
    public double utilizacion(){
        /*El método establece un double resultado de la division entre el tamaño del árbol
        y la cantidad total de espacios con los métodos size y nodos respectivamente.
         */
        double valor = (double) size()/nodos(root);
        //El resultado se retornará multiplicado por 100, mostrando el porcentaje al final.
        return valor*100;
    }
    //Método privado para saber el número de espacios totales que posee el árbol
    private int nodos(NodoArbolB x){
        ...
        /*Igual que en el método tamaño, comprobara si el nodo es una hoja o no, en caso de que no lo sea
        mediante recursividad procederá a revisar cada hijo del nodo y sumara el valor de sus tamaños con el
        de su padre
         */
        if(!x.leaf){
            for(int i=0; i<=x.n; i++){
        ...
        //retorna el resultado de la suma de espacios
        return valor;
    }
}